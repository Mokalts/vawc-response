...
if (!document.getElementById('vawc-mycases-css')) {
    ...
    s.textContent=`
        ...
        .vmc-report-row:hover { background:var(--surface-alt) !important; }
...
const Skel = () => (
    <div style={S.card}>
        {[['60%',12],['100%',11],['75%',11]].map(([w,h],i)=>(
            <div key={i} style={{width:w,height:h,borderRadius: 4,backgroundColor:'var(--border-soft)',marginBottom:i<2?10:0,animation:'shimmer 1.2s infinite'}}/>
        ))}
    </div>
);
...
function CaseCard({ cas, onClick }) {
    ...
    return (
        <div className="vmc-card" role="button" tabIndex={0}
            aria-label={`Kaso ${cas.case_number} laban kay ${cas.offender_name}, ${cas.status_display || st.label}. Buksan.`}
            onKeyDown={(e)=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); onClick(); } }}
            style={{...S.card, border: cas.has_status_update?'1.5px solid #F47920':'1px solid var(--border)'}} onClick={onClick}>
            {cas.has_status_update && (
                <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:8}}>
                    <span style={{width:8,height:8,borderRadius: '50%',background:'#F47920',flexShrink:0}}/>
                    <span style={{fontSize:11,fontWeight:700,color:'var(--accent-text)',fontFamily:"'Lexend',sans-serif"}}>Status Updated</span>
                </div>
            )}
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div style={{display:'flex',alignItems:'center',gap:12}}>
                    ...
                    <span style={S.meta}><IcoDoc c="#64748B" size={13}/> {cas.report_count} {cas.report_count===1?'report':'reports'}</span>
                </div>
...
function ReportRow({ report, index, onClick }) {
    return (
        <div className="vmc-report-row" role="button" tabIndex={0}
            aria-label={`Report ${index+1}, buksan ang detalye`}
            onKeyDown={(e)=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); onClick(); } }}
            onClick={onClick}
            style={{padding:'14px 16px',borderBottom:'1px solid var(--surface-alt)',cursor:'pointer',backgroundColor:'var(--surface)'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6}}>
                <div style={{display:'flex',alignItems:'center',gap:7}}>
                    <span style={{fontSize:11.5,fontWeight:700,color:'var(--accent-text)',background:'var(--surface-tint)',padding:'2px 8px',borderRadius: 4,fontFamily:"'Lexend',sans-serif"}}>
                        Report #{index+1}
                    </span>
                    {report.incident_type&&(
                        <span style={{fontSize:11,color:'#4A1259',background:'var(--surface-tint)',padding:'2px 7px',borderRadius: 4,fontFamily:"'Lexend',sans-serif",fontWeight:600}}>
                            {report.incident_type}
                        </span>
                    )}
                </div>
                <div style={{display:'flex',alignItems:'center',gap:4}}>
                    <IcoCal c="#64748B" size={11}/>
                    <span style={{fontSize:11.5,color:'var(--text-muted)',fontFamily:"'Lexend',sans-serif"}}>{fmtDate(report.created_at)}</span>
                </div>
            </div>
            <p style={{margin:0,fontSize:13.5,color:'var(--text-body)',lineHeight:1.5,fontFamily:"'Lexend',sans-serif"}}>
    ...
    return (
        <div style={S.backdrop} onClick={onClose}>
            <div style={S.modal} onClick={e=>e.stopPropagation()}
                role="dialog" aria-modal="true" aria-label={`Report ${index+1}`}>
                <div style={S.modalHead}>
                    ...
                    <button style={S.closeBtn} onClick={onClose} aria-label="Isara"><IcoX/></button>
                </div>
...
function CaseDetailModal({ cas, onClose, onStatusRead }) {
    ...
    useEffect(() => {
        const onKey = (e) => { if (e.key === 'Escape') { if (selectedReport) setSelectedReport(null); else onClose(); } };
    ...
    return (
        <div style={S.backdrop} onClick={onClose}>
            <div style={{...S.modal,maxWidth:480}} onClick={e=>e.stopPropagation()}
                role="dialog" aria-modal="true" aria-label={`Kaso ${cas.case_number}`}>
                <div style={S.modalHead}>
                    <div>
                        <p style={{...S.modalTitle,fontSize:14}}>{cas.case_number}</p>
                        <p style={{margin:'2px 0 0',fontSize:13,fontWeight:700,color:'var(--text)',fontFamily:"'Lexend',sans-serif"}}>vs. {cas.offender_name}</p>
                    </div>
                    <button style={S.closeBtn} onClick={onClose} aria-label="Isara"><IcoX/></button>
                </div>
                <div style={S.modalBody}>
                    ...
                    {cas.has_status_update && (
                        <div style={{display:'flex',gap:8,alignItems:'flex-start',backgroundColor:'var(--surface-tint)',borderRadius: 4,padding:'11px 13px',border:'1px solid var(--border)'}}>
                            <IcoBell/>
                            <p style={{fontSize:13,color:'var(--accent-text)',lineHeight:1.6,margin:0,fontFamily:"'Lexend',sans-serif"}}>
                    ...
                    {cas.messages && cas.messages.length > 0 && (
                        <div style={{backgroundColor:'#F3E5F5',borderRadius:12,padding:'14px 15px',border:'1.5px solid #E1BEE7'}}>
                            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                                <IcoBell c="#7B2D8B"/>
                                <p style={{margin:0,fontSize:13.5,fontWeight:800,color:'#4A1259',fontFamily:"'Lexend',sans-serif"}}>Messages from the Barangay ({cas.messages.length})</p>
                            </div>
                            <div style={{display:'flex',flexDirection:'column',gap:10}}>
                                {cas.messages.map(m => (
                                    <div key={m.id} style={{backgroundColor:'var(--surface)',borderRadius:8,padding:'10px 12px',border:'1px solid #E1BEE7'}}>
                                        <p style={{margin:0,fontSize:13.5,color:'#4A1259',lineHeight:1.6,whiteSpace:'pre-line',fontFamily:"'Lexend',sans-serif"}}>{m.message}</p>
                                        <p style={{margin:'6px 0 0',fontSize:11,color:'#9B4DAB',fontFamily:"'Lexend',sans-serif"}}>{fmtDate(m.created_at)}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {cas.status === 'awaiting_onsite_visit' && (
                        <div style={{backgroundColor:'var(--surface-tint)',borderRadius: 12,padding:'15px 16px',border:'1.5px solid #F47920'}}>
                            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                                <IcoPin c="#C45E10" size={17}/>
                                <p style={{margin:0,fontSize:13.5,fontWeight:800,color:'var(--accent-text)',fontFamily:"'Lexend',sans-serif"}}>
                                    Action Needed: Visit the VAWC Office
                                </p>
                            </div>
                            <p style={{margin:'0 0 10px',fontSize:13,color:'#7A3A0E',lineHeight:1.65,fontFamily:"'Lexend',sans-serif"}}>
                            ...
                            <div style={{display:'flex',flexDirection:'column',gap:5,backgroundColor:'var(--surface)',borderRadius: 8,padding:'11px 13px',border:'1px solid var(--border)'}}>
                                <span style={{display:'flex',alignItems:'center',gap:7,fontSize:12.5,color:'var(--text)',fontFamily:"'Lexend',sans-serif"}}>
                            ...
                            <p style={{margin:'10px 0 0',fontSize:11.5,color:'var(--text-muted)',lineHeight:1.55,fontFamily:"'Lexend',sans-serif"}}>
                    ...
                    {cas.reports&&cas.reports.length>0&&(
                        <div>
                            <p style={{margin:'0 0 8px',fontSize:11,fontWeight:700,color:'var(--accent-text)',textTransform:'uppercase',letterSpacing:'0.6px',fontFamily:"'Lexend',sans-serif"}}>
                                Reports / Testimonies ({cas.reports.length})
                            </p>
                            <div style={{backgroundColor:'var(--surface)',borderRadius: 0,border:'1px solid var(--border)',overflow:'hidden'}}>
                                {cas.reports.map((r,i)=>(
        ...
        <div style={S.page}>
            <header style={S.topBar}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                    {newCount>0&&(
                        <span style={{display:'inline-flex',alignItems:'center',gap:5,padding:'5px 12px',borderRadius: 9999,background:'var(--surface-tint)',color:'var(--accent-text)',fontSize:12,fontWeight:700,fontFamily:"'Lexend',sans-serif",border:'1px solid var(--border)'}}>
            ...
            {selectedCase&&(
                detailLoading
                    ? <div style={{position:'fixed',inset:0,backgroundColor:'rgba(15,23,42,0.45)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:500}}>
                        <div style={{width:40,height:40,border:'3px solid var(--surface-tint)',borderTopColor:'#F47920',borderRadius: '50%',animation:'spin 0.7s linear infinite'}}/>
                      </div>
...
const S = {
    ...
    topBar:      { display:'flex',justifyContent:'space-between',alignItems:'center',padding:'16px 20px',backgroundColor:'var(--surface)',borderBottom:'1px solid var(--border)',position:'sticky',top:0,zIndex:100,boxShadow:'0 1px 8px rgba(244,121,32,0.06)' },
    title:       { fontSize:18,fontWeight:800,color:'var(--accent-text)',fontFamily:"'Lexend',sans-serif",margin:'0 0 2px' },
    count:       { fontSize:12.5,fontWeight:600,color:'var(--text-muted)',fontFamily:"'Lexend',sans-serif",margin:0 },
    ...
    emptyTitle:  { fontSize:18,fontWeight:700,color:'var(--accent-text)',fontFamily:"'Lexend',sans-serif" },
    emptyText:   { fontSize:14,color:'var(--text-muted)',lineHeight:1.6,fontFamily:"'Lexend',sans-serif",maxWidth:280 },
    ...
    modal:       { backgroundColor:'var(--surface)',borderRadius: 16,width:'100%',maxWidth:420,maxHeight:'88vh',display:'flex',flexDirection:'column',boxShadow:'0 20px 60px rgba(15,23,42,0.2)',animation:'popIn 0.22s ease' },
    modalHead:   { display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 20px 14px',borderBottom:'1px solid var(--border-soft)',flexShrink:0 },
    modalBody:   { overflowY:'auto',padding:'16px 20px 24px',display:'flex',flexDirection:'column',gap:12 },
    modalTitle:  { fontSize:17,fontWeight:800,color:'var(--accent-text)',fontFamily:"'Lexend',sans-serif",margin:0 },
    closeBtn:    { width:44,height:44,borderRadius: 10,backgroundColor:'transparent',border:'none',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer' },
    detailItem:  { backgroundColor:'var(--surface-alt)',borderRadius: 0,padding:'14px 15px',border:'1px solid var(--border)' },
    detailLabel: { fontSize:10.5,fontWeight:700,color:'var(--accent-text)',textTransform:'uppercase',letterSpacing:'0.7px',margin:'0 0 6px',fontFamily:"'Lexend',sans-serif" },
    detailValue: { fontSize:14.5,color:'var(--text)',lineHeight:1.6,margin:0,fontFamily:"'Lexend',sans-serif" },
