...
const IcoEyeOff  = () => (<svg width="17" height="17" viewBox="0 0 24 24" fill="none"><path d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19m-6.72-1.07a3 3 0 11-4.24-4.24" stroke="#64748B" strokeWidth="1.8" strokeLinecap="round"/><line x1="1" y1="1" x2="23" y2="23" stroke="#64748B" strokeWidth="1.8" strokeLinecap="round"/></svg>);
    ...
    if (success) return (
        <div style={{ ...S.page, alignItems:'center', justifyContent:'center', padding:28 }}>
            <div style={{ textAlign:'center', maxWidth:340 }}>
                ...
                <h2 style={{ fontSize:20, fontWeight:800, color:'#059669', marginBottom:10, fontFamily:"'Lexend', sans-serif" }}>Password Updated!</h2>
                <p style={{ fontSize:14, color:'var(--text-body)', lineHeight:1.7, fontFamily:"'Lexend', sans-serif" }}>
    ...
    return (
        <div style={S.page}>
            <header style={S.topBar}>
                <button style={S.backBtn} onClick={() => navigate('/settings')} aria-label="Bumalik sa Settings"><IcoArrow /></button>
            ...
            <main style={S.content}>
                ...
                <div style={S.headerCard}>
                    ...
                    <div>
                        <p style={{ fontSize:15, fontWeight:700, color:'var(--accent-text)', fontFamily:"'Lexend', sans-serif", marginBottom:2 }}>Update your password</p>
                        <p style={{ fontSize:13, color:'var(--text-muted)', fontFamily:"'Lexend', sans-serif" }}>Choose a strong password to keep your account safe.</p>
                    </div>
                </div>
                ...
                <div style={S.card}>
                    ...
                    {form.new_password.length > 0 && (
                        <div style={{ marginBottom:14, animation:'fadeUp 0.2s ease' }}>
                            <div style={{ display:'flex', gap:4, marginBottom:5 }}>
                                {[1,2,3,4].map(i => (
                                    <div key={i} style={{ flex:1, height:4, borderRadius: 4, backgroundColor: i<=strength ? STRENGTH_COLOR[strength] : 'var(--border-soft)', transition:'background-color 0.2s' }} />
                                ))}
                            </div>
                            <p style={{ fontSize:12, color: STRENGTH_COLOR[strength], fontWeight:600, fontFamily:"'Lexend', sans-serif" }}>
                        ...
                        {REQUIREMENTS.map(req => {
                            ...
                            return (
                                <div key={req.label} style={{ display:'flex', alignItems:'center', gap:6 }}>
                                    <div style={{ width:16, height:16, borderRadius: 4, backgroundColor: met?'#ECFDF5':'var(--border-soft)', border:`1.5px solid ${met?'#6EE7B7':'var(--border)'}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'all 0.15s' }}>
                                        {met && <IcoCheck c="#059669" size={9} />}
                                    </div>
                                    <p style={{ fontSize:11.5, color: met?'#059669':'#64748B', fontFamily:"'Lexend', sans-serif", transition:'color 0.15s' }}>{req.label}</p>
                                </div>
                            );
                        })}
                    </div>
                    ...
                    {form.confirm_password && (
                        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4, animation:'fadeUp 0.15s ease' }}>
                            {passwordsMatch
                                ? <><IcoCheck c="#059669" size={13} /><p style={{ fontSize:12, color:'#059669', fontFamily:"'Lexend', sans-serif" }}>Passwords match</p></>
                                : <><IcoWarn c="#C62828" /><p style={{ fontSize:12, color:'#C62828', fontFamily:"'Lexend', sans-serif" }}>Passwords do not match</p></>}
                        </div>
                    )}
...
const S = {
    ...
    label:      { fontSize:11, fontWeight:700, color:'var(--accent-text)', textTransform:'uppercase', letterSpacing:'0.07em', fontFamily:"'Lexend', sans-serif" },
    pwBox:      { display:'flex', alignItems:'center', border:'1.5px solid var(--border)', borderRadius: 4, backgroundColor:'var(--surface-alt)', overflow:'hidden' },
    ...
    submitBtn:  { width:'100%', padding:'14px', backgroundColor:'#F47920', color:'#fff', border:'none', borderRadius: 8, fontSize:15, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:9, fontFamily:"'Lexend', sans-serif", boxShadow:'0 2px 10px rgba(244,121,32,0.3)' },
};
